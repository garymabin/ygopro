		...
		if (length == 0) {
			...
			if (blockid == 0xffff) {
				...
				if (img.LoadMemory(imgbuf, length)) {
					...
					card_image.Load(img.GetRawData(), img.GetWidth(),
							img.GetHeight());
					...
					glBufferSubData(GL_ARRAY_BUFFER, 0,
							sizeof(glbase::v2ct) * 4, &frame_verts);
...
void ImageMgr::InitTextures(const std::wstring& image_path) {
	...
	glBindBuffer(GL_ARRAY_BUFFER, card_buffer[0]);
	GLCheckError(__FILE__, __LINE__);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glbase::v2ct) * 4, nullptr,
	GL_DYNAMIC_DRAW);
	GLCheckError(__FILE__, __LINE__);
	...
	GLCheckError(__FILE__, __LINE__);
	glVertexAttribPointer(1, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(glbase::v2ct),
			(const GLvoid*) glbase::v2ct::color_offset);
	GLCheckError(__FILE__, __LINE__);
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(glbase::v2ct),
			(const GLvoid*) glbase::v2ct::tex_offset);
	GLCheckError(__FILE__, __LINE__);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, card_buffer[1]);
	GLCheckError(__FILE__, __LINE__);
	glBindVertexArray(0);
	...
	FileSystem::TraversalDir(image_path,
			[&image_path, &image_files](const std::wstring& name, bool isdir) {
...
bool ImageMgr::LoadImageConfig(const std::wstring& name) {
		...
		if (config_name == "image") {
			...
			if (FileSystem::IsFileExists(path)) {
				...
				if (img.LoadFile(path)) {
					if (name == "card") {
						LOGI("enter load card");
						card_texture.Update(img.GetRawData(), 0, 0,
								img.GetWidth(), img.GetHeight());
						LOGI("bg_texture: %f%f, %f%f, %f%f, %f%f",
								card_texture.GetTextureInfo().vert[0].x,
								card_texture.GetTextureInfo().vert[0].y,
								card_texture.GetTextureInfo().vert[1].x,
								card_texture.GetTextureInfo().vert[1].y,
								card_texture.GetTextureInfo().vert[2].x,
								card_texture.GetTextureInfo().vert[2].y,
								card_texture.GetTextureInfo().vert[3].x,
								card_texture.GetTextureInfo().vert[3].y);
					} else if (name == "misc") {
						LOGI("enter load misc");
						misc_texture.Load(img.GetRawData(), img.GetWidth(),
								img.GetHeight());
						LOGI("misc_texture: %f%f, %f%f, %f%f, %f%f",
								misc_texture.GetTextureInfo().vert[0].x,
								misc_texture.GetTextureInfo().vert[0].y,
								misc_texture.GetTextureInfo().vert[1].x,
								misc_texture.GetTextureInfo().vert[1].y,
								misc_texture.GetTextureInfo().vert[2].x,
								misc_texture.GetTextureInfo().vert[2].y,
								misc_texture.GetTextureInfo().vert[3].x,
								misc_texture.GetTextureInfo().vert[3].y);
					} else if (name == "bg") {
						LOGI("enter load bg");
						bg_texture.Load(img.GetRawData(), img.GetWidth(),
								img.GetHeight());
						LOGI("bg_texture: %f%f, %f%f, %f%f, %f%f",
								bg_texture.GetTextureInfo().vert[0].x,
								bg_texture.GetTextureInfo().vert[0].y,
								bg_texture.GetTextureInfo().vert[1].x,
								bg_texture.GetTextureInfo().vert[1].y,
								bg_texture.GetTextureInfo().vert[2].x,
								bg_texture.GetTextureInfo().vert[2].y,
								bg_texture.GetTextureInfo().vert[3].x,
								bg_texture.GetTextureInfo().vert[3].y);
					}
				}
			}
			...
			if (ptex) {
				...
				LOGI(
						"ti: ti.vert[0].x＝%f,ti.vert[0].y=%f, "
								"ti.vert[1].x=%fti.vert[1].y=%f,"
								" ti.vert[2].x=%f, ti.vert[2].x=%f, ti.vert[3].x=%f, ti.vert[3].x=%f",
						ti.vert[0].x, ti.vert[0].y, ti.vert[1].x, ti.vert[1].y,
						ti.vert[2].x, ti.vert[2].y, ti.vert[3].x, ti.vert[3].y);
			}
	...
	for (int i = 0; i < 16; ++i) {
		int x = i % 4;
		int y = i / 4;
		char_textures[i].vert[0] = char_tex.vert[0]
				+ v2f { difx * x, dify * y };
		char_textures[i].vert[1] = char_tex.vert[0]
				+ v2f { difx * (x + 1), dify * y };
		char_textures[i].vert[2] = char_tex.vert[0]
				+ v2f { difx * x, dify * (y + 1) };
		char_textures[i].vert[3] = char_tex.vert[0]
				+ v2f { difx * (x + 1), dify * (y + 1) };
	}
	return true;
}
